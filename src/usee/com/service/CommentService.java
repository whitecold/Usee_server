...
public class CommentService {
	 ...
	 * @return
	 */
public String saveComment(String postid,String replyid,String messageid,String content){
	//检查评论的消息是否可用
	...
	if(error==0){
		...
		sql="update message set commentnum=commentnum+1 where id='"+messageid+"'";
		DbUpdate.update(sql);
		json.put("error", 0);
 ...
 * @return
 */
public String praise(String postid ,String messageid){
	//判断该消息是否可用
	JSONObject json=new JSONObject();
	String url=Api.GetUrl+messageid;
	String back="";
	...
	//如果可用添加更新点赞数目
	if(error==0){
		String sql="update message set praisenum=praisenum+1 where id='"+messageid+"'";
		DbUpdate.update(sql);
		json.put("error", 0);
		json.put("result", "success");
		
		RemindService rs=new RemindService();
		rs.saveRemind(postid, "0", messageid, "praise", "");
	}
	//否则返回错误的提示信息
	else {
		json.put("error", 1);
