...
public class MessageService {
	...
	public String getMessages(String lon,String lat){
		...
		String sql="select * from message where lon like '"+lon+"%' and lat like '"+lat+"%' and station='true'";
...
//			json.put("address", address);
			...
			//如果不可用将数据库该记录标记为不可用
				...
				DbUpdate.update("update message set station='false' where id='"+mess.getId()+"'");
			}
		} catch (Exception e) {
	 ...
	 * @return
	 */
	public boolean saveMessage(String content,String lon,String lat,String deadline,String devid,String kind){
//		logger.info("hello");
		try {
			//保存消息到阅后即焚服务器
...
//		json.put("lat",mess.getLat());
		...
		//否则返回错误提示信息
			...
			DbUpdate.update("update message set station='false' where id='"+id+"'");
			JSONObject json=new JSONObject();
			json.put("error", 1);
			json.put("content", content);
			return json.toString();
